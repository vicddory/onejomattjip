...
COLOR_STABLE_GRAY = "#757575"  # 안정 (회색)
...
# --- 10. 헤더 ---
st.markdown("<h1 style='text-align: center;'>커피 무역 인사이트 & FAQ</h1>", unsafe_allow_html=True)
st.divider()
...
with tab1:
    ...
    with c1:
        st.markdown(
            f"<h4 style='color:#6F4E37;'>🌍 {selected_year} 국가별 커피 생두 수입 비중</h4>",
            unsafe_allow_html=True
        )
        fig = px.pie(
            f_import,
    ...
    with c2:
        st.markdown(
            f"<h4 style='color:#6F4E37;'>📊 {selected_year} 실측 데이터</h4>",
            unsafe_allow_html=True
        )
        st.dataframe(
            f_import[['Country', 'Import_Qty', 'Value_USD', 'Region']]
            .sort_values("Import_Qty", ascending=False),
            column_config={
                "Import_Qty": st.column_config.ProgressColumn(
                    "수입량(ton)", format="%.1f", min_value=0, max_value=60000
                ),
